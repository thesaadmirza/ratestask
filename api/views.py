...
def rates_api(request):
    ...
    if date_from and date_to and origin and destination:
        ...
        while date_from <= date_to:
            cursor.execute(
                "SELECT AVG(price) as avgprice  FROM prices JOIN ports ON prices.orig_code=ports.code WHERE (ports.code=%s OR ports.parent_slug=%s) AND (ports.code=%s OR ports.parent_slug=%s) AND day=%s",
